...
CPivxlStake* CPivxlStake::NewPivStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CPivxlStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new CPivxlStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new CPivxlStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}

bool CPivxlStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

CTxIn CPivxlStake::GetTxIn() const
{
    return CTxIn(outpointFrom.hash, outpointFrom.n);
}

CAmount CPivxlStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CPivxlStake::CreateTxOuts(const CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal) const
{
...
CDataStream CPivxlStake::GetUniqueness() const
{
    //The unique identifier for a PIVXL stake is the outpoint
...
const CBlockIndex* CPivxlStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CPivxlStake: uninitialized pindexFrom");
    return pindexFrom;
}
