...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("PIVXLMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("PIVXLMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("PIVXLMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("pivxl-miner");
    const Consensus::Params& consensus = Params().GetConsensus();
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running PIVXLMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("PIVXLMiner exception");
    } catch (...) {
        LogPrintf("PIVXLMiner exception");
    }

    LogPrintf("PIVXLMiner exiting\n");
}
