...
// Copyright (c) 2015-2019 The PIVXL developers
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version")) {
        std::string strUsage = _("PIVXL Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  pivxl-cli [options] <command> [params]  " + _("Send command to PIVXL Core") + "\n" +
                        "  pivxl-cli [options] help                " + _("List commands") + "\n" +
                        "  pivxl-cli [options] help <command>      " + _("Get help for a command") + "\n";
