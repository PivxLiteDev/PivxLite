...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select PIVXL Outputs to Spend" :
            "Select Shielded PIVXL to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::PIVXL;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show PIVXL address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
