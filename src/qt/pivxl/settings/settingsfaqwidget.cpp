...
SettingsFaqWidget::SettingsFaqWidget(PIVXLGUI* parent, ClientModel* _model) :
    QDialog(parent),
    ...
    clientModel(_model)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendablePIVXL,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendablePIVXL,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendablePIVXL,
    ...
    setCssProperty({
              ui->pushButton_Intro,
              ui->pushButton_UnspendablePIVXL,
              ui->pushButton_Stake,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("Pivx Lite is a hybrid POS/MN coin that exists with the sole aim of becoming "
               "one the most widely utilized privacy-based coins in the market. The stated "
               "mission of Pivx Lite is to be an open source, scalable, cryptocurrency "
        ...
        formatFAQParagraph(
            tr("We endeavor to make it impossible for a third party to figure out through chain analysis "
               ...
               "For more information read the Pivx Lite Whitepaper located here %1 ")
                .arg("<a style='color: #b088ff' href='https://pivxlite.com/Pivx_Lite_Whitepaper_v1.0.pdf'>" + tr("https://pivxlite.com/Pivx_Lite_Whitepaper_v1.0.pdf") + "</a>.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendablePIVContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received PIVXL requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your PIVXL wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendablePIVXL->setText(unspendablePIVContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of PIVXL with a minimum of 300 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("pivxl-cli", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ui->labelContent_Stake->setText(stakeContent);

    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1  if you prefer to submit a ticket, you can do so at pivxlite@protonmail.com")
                .arg("<a style='color: #b088ff' href='https://discord.gg/c26QuZw'>" + tr("#support in our Discord") + "</a>.")));
    ...
    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node %1 wallet with a "
               ...
               "regularly. These services include:")
                .arg(PACKAGE_NAME)
                .arg(GUIUtil::formatBalance(clientModel->getMNCollateralRequiredAmount(), BitcoinUnits::PIVXL)) +
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in PIVXL Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the PIVXL network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
    ...
    connect(ui->pushButton_UnspendablePIVXL, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendablePIVXL);});
