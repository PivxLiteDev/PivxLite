...
void PrepareShutdown()
{
    ...
    util::ThreadRename("pivxl-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the PIVXL and zPIVXL money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-zpivstake=<n>", strprintf(_("Enable or disable staking functionality for zPIVXL inputs (0-1, default: %u)"), 1));
...
{
    util::ThreadRename("pivxl-loadblk");
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. PIVXL Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. PIVXL Core is probably already running."), strDataDir));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (!zwalletMain->GetMasterSeed().IsNull()) {
            ...
            uiInterface.InitMessage(_("Syncing zPIVXL wallet..."));
