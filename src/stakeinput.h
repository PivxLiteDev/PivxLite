...
class CPivxlStake : public CStakeInput
{
private:
...
public:
    CPivxlStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CPivxlStake* NewPivStake(const CTxIn& txin, int nHeight, uint32_t nTime);
