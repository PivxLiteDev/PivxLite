...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/PIVXLParams"
    else
        PARAMS_DIR="$HOME/.pivxl-params"
    fi
fi
...
Failed to fetch the PIVXL zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
PIVXL - fetch-params.sh

This script will fetch the PIVXL zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common PIVXL zkSNARK parameters. Note that it is
