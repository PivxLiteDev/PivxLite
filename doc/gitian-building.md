...
*Setup instructions for a gitian build of PIVXL Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the PIVXL
...
to the PIVXL GitHub release page.
...
- [Building PIVXL Core](#building-pivxl-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/pivxl-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/pivxl-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/pivxl-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/pivxl-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/pivxl-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/pivxl-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/pivxl-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@pivxlite.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/pivxl-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/pivxl-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/pivxl-osx-*-build.assert
```
