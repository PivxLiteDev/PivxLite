...
Other implementations of the zerocoin protocol only allow for spending of one denomination/zerocoin at a time. The PIVXL implementation of zerocoin allows users to spend any amount of zPIV they would like (with certain limitations). If a user held two denominations of 5 and they send 7.75 to a merchant, the wallet will automatically grab the two denominations of 5 and then issue 2.25 PIVXL in change to the spender. There is currently a limit of up to 6 individual zerocoin `coins` that can be combined into a spend, where each `coin` could be a different or similar denomination
...
**GUI** - Conversion from zPIV to PIVXL can be done using the `Privacy Dialog` in the QT wallet. Enter a PIVXL address that you would like to Pay To, enter the amount of PIVXL the receiver should be sent, click `Spend Zerocoin`.
...
### Broad Features
- #264 `15e84e5` zPIV is here! (Fuzzbawls Mrs-X Presstab Spock PIVXL)
