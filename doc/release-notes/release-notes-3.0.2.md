...
PIVX Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zPIVXL spends, automint calculation adjustments, and other various updates/fixes.
...
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in zPIVXL mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

zPIVXL Spending Fix
---------------------
The size of zPIVXL spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

zPIVXL Transaction Recovery
---------------------
Due to the aforementioned issue with zPIVXL spending, users may find that their attempted spends are now conflicted and zPIVXL balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
