PIVXL version *v4.0.0* is now available from:  <https://github.com/pivxl-project/pivxl/releases>
...
PIVXL v4.0.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
You can read more details about this extensive work in ([PR #954](https://github.com/PIVXL-Project/PIVXL/pull/954))
...
A brand new feature is being introduced with the release of v4.0.0: Cold Staking ([PR #955](https://github.com/PIVXL-Project/PIVXL/pull/955))! This feature allows a coin owner to keep coins in a "cold" (or locked) wallet whilst a "hot" wallet carries out the burden of staking those coins.
...
A full technical writup is available on the [PIVXL Wiki](https://github.com/PIVXL-Project/PIVXL/wiki/ColdStaking), and an initial video showcase is available on [YouTube](https://www.youtube.com/watch?v=utxB5TzAeXc).
A brief guide to setup cold staking with GUI and RPC is available [here](https://github.com/random-zebra/PIVXL-Wiki/blob/master/User-Documentation/Cold-Staking-HowTo.md).

<sup>1</sup> Spending cold stakes from HW wallets currently available only for Ledger devices via [PET4L](https://github.com/PIVXL-Project/PET4L) tool.
...
[#PR1022](https://github.com/PIVXL-Project/PIVXL/pull/1022) defines Version 7 blocks, which remove the (now-unused) accumulator checkpoint from the block header. This results in an overall data reduction of ~256 bits from each block as well as the in-memory indexes.

### New Network Message Signatures

Layer 2 network messages (MN, Budget, Spork, etc) are now signed based on the hash of their **binary** content instead of their **string** representation ([#PR1024](https://github.com/PIVXL-Project/PIVXL/pull/1024)).

### New SPORKS

Two new SPORKS are introduced, `SPORK_17` ([#PR975](https://github.com/PIVXL-Project/PIVXL/pull/975)) and `SPORK_18` ([#PR995](https://github.com/PIVXL-Project/PIVXL/pull/995)).<br>
...
`SPORK_18` (off by default) is used to switch between Version 3 and [Version 4 Public Spends](#v1-zpivxl-spending-public-spends-version-4). When this spork is active, only version 4 spends are accepted by the network. When it's not, only version 3 spends are accepted.
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~pivxl) repository, we are now offering a [Snap package](https://snapcraft.io/pivxl-core) as quick way to install and update a PIVXL wallet.
...
- #1129 `a87bfc32a0` [Consensus] Define TestNet changeover block for PIVXL v4.0 RC (random-zebra)
- #1134 `a4ded20de4` [Trivial] Remove a duplicate variable definition (warrows)
- #1191 `9a054eeba6` [Consensus] Define MainNet changeover block for PIVXL v4.0 (random-zebra)
...
- #1179 `7a33fe10d2` [Qt] Fix for dead link to wrong PIVXL website (NoobieDev12)
