PIVXL version 3.0.4 is now available from:
...
PIVXL v3.0.4 is a mandatory update for all users. This release contains various updates/fixes pertaining to the zPIVXL protocol, supply tracking, block transmission and relaying, as well as usability and quality-of-life updates to the GUI.
...
Refactoring of zPivxl Spend Validation Code
---------------------
zPivxl spend validation was too rigid and did not give enough slack for reorganizations. Many staking wallets were unable to reorganize back to the correct blockchain when they had an orphan stake which contained a zPivxl spend. zPivxl double spending validation has been refactored to properly account for reorganization.

Money Supply Calculation Fix
---------------------
Coin supply incorrectly was counting spent zPivxl as newly minted coins that are added to the coin supply, thus resulting in innacurate coin supply data.
...
---------------------
The stake miner code now filters out zPivxl double spends that were on rare occasions being slipped into blocks (and being rejected by peers when broadcast to the network).
...
More Extensive Display of zPivxl Confirmation/Maturity Status
---------------------
Within the Privacy Dialog of the QT wallet, the _Zerocoin Stats_ section now displays both the confirmation and maturity status of minted zPivxl's. Previously this was only able to be viewed within the zPivxl Control dialog. The main Overview tab of the QT wallet now splits zPivxl balance into subcategories: _Unconfirmed_, _Immature_, and _Mature_.
...
- #294 `27c0943` Add additional checks for txid for zpivxl spend. (presstab)
- #301 `b8392cd` Refactor zPivxl tx counting code. Add a final check in ConnectBlock() (presstab)
- #306 `77dd55c` [Core] Don't send not-validated blocks (Mrs-X)
- #312 `5d79bea` [Main] Update last checkpoint data (Fuzzbawls)
- #325 `7d98ebe` Reindex zPivxl blocks and correct stats. (presstab)
