...
You can find the PIVX translation project at [https://www.transifex.com/pivxl-project/pivxl-project-translations/](https://www.transifex.com/pivxl-project/pivxl-project-translations/).
...
3. Update `src/qt/pivxl_locale.qrc` manually or via
```bash
git ls-files src/qt/locale/*ts|xargs -n1 basename|sed 's/\(pivxl_\(.*\)\).ts/<file alias="\2">locale\/\1.qm<\/file>/'
```
4. Update `src/Makefile.qt.include` manually or via
```bash
git ls-files src/qt/locale/*ts|xargs -n1 basename|sed 's/\(pivxl_\(.*\)\).ts/  qt\/locale\/\1.ts \\/'
```
...
1. Open `pivxl_en.ts` in Qt Linguist (included in the Qt SDK)
...
To create a new language template, you will need to edit the languages manifest file `src/qt/pivxl_locale.qrc` and add a new entry. Below is an example of the English language entry.
...
The PIVX Core translation maintainers include *Fuzzbawls*. You can find them, and others, in the [PIVX Discord](https://discord.pivxl.org).
